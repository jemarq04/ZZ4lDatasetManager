...
class ConfigHistFactory(object):
    def __init__(self, dataset_manager_path, dataset_name, object_restrict=""):
        ...
        self.styles = ConfigHistTools.readInfo("/".join([self.manager_path, "Styles", "styles.json"]))
        ...
        object_file = "/".join(
            [
                self.manager_path,
                "PlotObjects",
                ("_".join([self.dataset_name, object_restrict]) if object_restrict != "" else self.dataset_name)
                + ".py",
            ]
        )
    ...
    def readAllInSet(self, object_type, base_name):
        info = ConfigHistTools.readAllInfo("/".join([self.manager_path, object_type, "%s*" % base_name]))
        return info
    ...
    def getHistDrawExpr(self, object_name, dataset_name, channel):
        hist_name = "_".join([x for x in [dataset_name, channel, object_name] if x != ""])
    ...
    def get2DHistDrawExpr(self, xobject_name, yobject_name, dataset_name, channel):
        hist_name = "_".join([x for x in [dataset_name, channel, xobject_name, yobject_name] if x != ""])
    ...
    def setHistAttributes(self, hist, object_name, plot_group):
        ...
        plot_group = (
            self.plot_groups[info[plot_group]["plot_group"]]
            if plot_group not in list(self.plot_groups.keys())
            else self.plot_groups[plot_group]
        )
    ...
    def addErrorToHist(self, hist, plot_group_name):
        # If not a valid plot group, try treating it as file entry
        plot_group = (
            self.plot_groups[self.info[plot_group_name]["plot_group"]]
            if plot_group_name not in list(self.plot_groups.keys())
            else self.plot_groups[plot_group_name]
        )
